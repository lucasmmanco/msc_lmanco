...
static int32_t istream_callback(void * buf, size_t buf_len, void * context)
{
    ...
    if (copy_size)
    {
        //TT mandando o arquivo para o fpga
        ...
        void * user_data)
{
    ...
    do
    {
        ...
        if (cb_status > 0)
        {
            ...
            //TT achei que funcao acima já mandava os dados
            //para o fpga, não tenho certeza do que esta 
            //acontecendo aqui, talvez alguma forma de config?
...
ALT_STATUS_CODE test_config_full(char *filename, void *virtual_base)
{
    ...
    //TT o que são MSELs?
    //MSELS parecem ser pinos que decidem o modo de configuração do fpga
...
{  
    ...
    // TTverificando se o script foi chamado corretamente
    ...
    // TT verificando acesso a /dev/mem
    // entendo que isso é a CRAM
    if( ( fd = open("/dev/mem", ( O_RDWR | O_SYNC ) ) ) == -1 ) {
        printf( "ERROR: could not open \"/dev/mem\"...\n" );
        return( 1 );
    }

    // TT cria um endereco base da CRAM 
    ...
    //TT setta alguns enderecos a partir do endereco base
    //do bloco na CRAM
    //o que diabos é HW_REGS_MASK ???
    ...
    //permite que o hps controle o bloco de controle do fpga
