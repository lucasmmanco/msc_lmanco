...
#define GP_REG_ADDR   ( 0xff200100 )
#define IN1_REG_ADDR  ( 0xff200110 )
#define IN2_REG_ADDR  ( 0xff200120 )
#define IN3_REG_ADDR  ( 0xff200130 )
#define IN4_REG_ADDR  ( 0xff200140 )
#define OUT1_REG_ADDR ( 0xff200200 )
#define OUT2_REG_ADDR ( 0xff200210 )
#define OUT3_REG_ADDR ( 0xff200220 )
...
{  
    ...
    int32_t a, b, c, op4, op5, op6, op8, x1, x2;

    filename[0] = "baskara_g1.rbf";
    filename[1] = "baskara_g2.rbf";
    filename[2] = "baskara_g3.rbf";
	...
	if(argc!=4)
	{
		printf("Please specify the indices of the second grade equation.\r\n");
		return 3;
	}

    while(i<3){
        if(access(filename[i], R_OK)!=0){
	...
	if (status == ALT_E_SUCCESS)
	{
	    printf("alt_fpga_control_enable OK  next config the fpga\n");
            status = test_config_full(filename[0]);
	}
	printf("INFO: alt_fpga_control_disable().\n");
	alt_fpga_control_disable();
	
    //set the indices of the second grade equation
	reg_addr=virtual_base + ( ( unsigned long  )( IN1_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, a);
	reg_addr=virtual_base + ( ( unsigned long  )( IN2_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, b);
	reg_addr=virtual_base + ( ( unsigned long  )( IN3_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, c);
	reg_addr=virtual_base + ( ( unsigned long  )( GP_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, 1);

    //read the resuts
	reg_addr=virtual_base + ( ( unsigned long  )( OUT1_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    op4 = alt_read_word(reg_addr);
	...
	{
	    printf("alt_fpga_control_enable OK  next config the fpga\n");
            status = test_config_full(filename[1]);
	}
	printf("INFO: alt_fpga_control_disable().\n");
	alt_fpga_control_disable();
    ...
    alt_write_word(reg_addr, b);
	reg_addr=virtual_base + ( ( unsigned long  )( IN2_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, op4);
	reg_addr=virtual_base + ( ( unsigned long  )( GP_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, 1);

    //read the resuts
	reg_addr=virtual_base + ( ( unsigned long  )( OUT1_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //alt_h2f_ofst + onchip_memory2_0_base + 
    op5 = alt_read_word(reg_addr);
	reg_addr=virtual_base + ( ( unsigned long  )( OUT2_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //alt_h2f_ofst + onchip_memory2_0_base + 
    op6 = alt_read_word(reg_addr);
	reg_addr=virtual_base + ( ( unsigned long  )( OUT3_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //alt_h2f_ofst + onchip_memory2_0_base + 
    op8 = alt_read_word(reg_addr);
	...
	{
	    printf("alt_fpga_control_enable OK  next config the fpga\n");
            status = test_config_full(filename[2]);
	}
	printf("INFO: alt_fpga_control_disable().\n");
	alt_fpga_control_disable();
    ...
    alt_write_word(reg_addr, a);
	reg_addr=virtual_base + ( ( unsigned long  )( IN2_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, op5);
	reg_addr=virtual_base + ( ( unsigned long  )( IN3_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, op6);
	reg_addr=virtual_base + ( ( unsigned long  )( IN4_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, op8);
    reg_addr=virtual_base + ( ( unsigned long  )( GP_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    alt_write_word(reg_addr, 1);

    //read the resuts
	reg_addr=virtual_base + ( ( unsigned long  )( OUT1_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    x1 = alt_read_word(reg_addr);
	reg_addr=virtual_base + ( ( unsigned long  )( OUT2_REG_ADDR ) & ( unsigned long)( HW_REGS_MASK ) ); //ALT_H2F_OFST + ONCHIP_MEMORY2_0_BASE + 
    x2 = alt_read_word(reg_addr);
